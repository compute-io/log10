...
describe( 'matrix log10', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			log10( matrix( [10,10] ), mat );
		}
	});

	it( 'should compute the base-10 logarithm for each matrix element', function test() {
		var actual;

		actual = matrix( [5,5], 'int16' );
		actual = log10( actual, mat );
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( log10( out, mat ).data, expected );
